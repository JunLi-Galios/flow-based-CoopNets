...
@torch.enable_grad()
def train_full(epoch, net, trainloader, device, optimizer, scheduler, loss_fn, max_grad_norm, iter_step):
    print('\nEpoch: %d' % epoch)
    net.train()
...
def test(epoch, net, testloader, device, loss_fn, num_samples):
    ...
    # Save checkpoint
    if loss_meter.avg < net.best_loss:
        print('Saving...')
        state = {
            'net': net.state_dict(),
